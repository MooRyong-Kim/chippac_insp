...
//---------------------------------------------------------------------------
//타워램프 선택
const int TWR_IDLE          = 0; //생산 모드를 제외한 다른 영역에서 정지되어있는 경우
const int TWR_START         = 1; //생산 모드에서 작업을 하고 있을 때
const int TWR_STOP          = 2; //생산 모드에서 정지된 경우
const int TWR_ALARM         = 3; //생산 모드에서 알람이 발생한 경우
const int TWR_CHANGE        = 4; //생산 모드에서 매거진을 교체 해야 될 경우
const int TWR_END           = 5; //생산 모드에서 작업 완료 시
const int TWR_MAX           = 6;
//---------------------------------------------------------------------------
